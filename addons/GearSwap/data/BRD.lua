...
function check_song()
	if state.AutoSongMode.value then
		...
		if not buffactive.minuet then
			windower.chat.input('/ma "Valor Minuet V" <me>')
			-- windower.chat.input('/ma "Sword Madrigal" <me>')
			tickdelay = os.clock() + 2
			return true
		end
		if not buffactive.madrigal then
			-- windower.chat.input('/ma "Gold Capriccio" <me>') -- for skillup cheatz
		end
			return false
	else
		return false
