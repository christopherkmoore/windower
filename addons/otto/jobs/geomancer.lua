...
local function check_bubble_out_of_range()
    ...
    if distance_between_old_bubble_and_target and distance_between_old_bubble_and_target >
        (user_settings.job.geomancer.bubble.distance or 10) then
...
local function check_aspir()
    if otto.aspir.ready.target and otto.aspir.ready.spell and otto.cast.is_mob_valid_target(otto.aspir.ready.target, otto.aspir.ready.spell.range) then
...
local function check_spells()
    ...
    if otto.player.is_moving or otto.player.mage_disabled() then
        return
    end
    local loupan = windower.ffxi.get_mob_by_target('pet')
    ...
    if geomancer.should_full_circle and loupan then
        local ja_ability = use_full_circle()
        if ja_ability then
            otto.cast.job_ability(ja_ability, '<me>') -- I keep seeing spam in chat fromt his... but it goes through recast.
    ...
    if not geomancer.indi.info.active then
        otto.cast.spell_no_check(indi_action, '<me>')
        return
    end
    ...
    if (geomancer.blaze_active or geomancer.ecliptic_attrition_active) and user_settings.job.geomancer.cooldowns then
        ...
        if protection_ability and loupan then
            otto.cast.job_ability(protection_ability, '<me>')
            return
        end
    end
    ...
    if not geomancer.ecliptic_attrition_active and user_settings.job.geomancer.cooldowns then
        ...
        if ecliptic_attrition_recast == 0 and loupan ~= nil then
            local ecliptic_attrition = {
                id = 347,
                en = "Ecliptic Attrition",
                ja = "サークルエンリッチ",
                element = 6,
                icon_id = 46,
                mp_cost = 0,
            ...
            otto.cast.job_ability(ecliptic_attrition, '<me>')
    ...
    if user_settings.job.geomancer.entrust and user_settings.job.geomancer.entrust.target and
        user_settings.job.geomancer.entrust.indi then
        local action = {
            id = 386,
            en = "Entrust",
            ja = "エントラスト",
        ...
        if target and entrust_recast == 0 or buffs:contains(action.status) then
            ...
            otto.cast.spell_with_pre_action(spell, action, target)
            return
        end
    end
    ...
    if geomancer.bubble_should_cast and geomancer.bubble_action and geomancer.bubble_target then
        if blaze_recast == 0 and user_settings.job.geomancer.cooldowns then
            ...
            otto.cast.spell_with_pre_action(geomancer.bubble_action, blaze, geomancer.bubble_target)
            geomancer.blaze_active = true
            return
        else
            otto.cast.spell(geomancer.bubble_action, geomancer.bubble_target)
    ...
    if otto.magic_burst.window_open_spell and otto.magic_burst.window_open_target then
        ...
        local collimated_ferver = {
            id = 348,
            en = "Collimated Fervor",
            ja = "コリメイトファーバー",
            duration = 60,
            element = 7,
            icon_id = 47,
        ...
        if user_settings.job.geomancer.cooldowns and theurgic_focus_recast == 0 and
            (not buffs:contains(theurgic_focus.status) or not buffs:contains(collimated_ferver.status)) then
            otto.cast.spell_with_pre_action(otto.magic_burst.window_open_spell, theurgic_focus,
                otto.magic_burst.window_open_target)
            return
        end

        if user_settings.job.geomancer.cooldowns and collimated_ferver_recast == 0 and
            (not buffs:contains(theurgic_focus.status) or not buffs:contains(collimated_ferver.status)) then
            otto.cast.spell_with_pre_action(otto.magic_burst.window_open_spell, collimated_ferver,
                otto.magic_burst.window_open_target)
            return
        end

        otto.cast.spell(otto.magic_burst.window_open_spell, otto.magic_burst.window_open_target)
        return
    end
...
function geomancer.check_geo()
    if not user_settings.job.geomancer.enabled then
        return
    end
    if otto.player.is_moving or otto.player.mage_disabled() then
        return
    ...
    if geomancer.counter >= otto.player.delay then
        geomancer.counter = 0
        otto.player.delay = geomancer.check_interval
